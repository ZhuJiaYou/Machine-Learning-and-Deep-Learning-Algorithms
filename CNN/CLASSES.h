...
public:
	...
	int ok_to_test;  // Set when model is trained, reset if user changes achitecture
...
private:
	...
	// These 'No threading' routines are not normally used; they can be forced at the end of MODEL.cpp for test
	...
	int n_layers;  // Number of layers, not include final
	int layer_type[MAX_LAYERS];  // Each entry (input to final) is TYPE_? in CONST.h
	int height[MAX_LAYERS];  // Number of neurons vertically in a slice of this layer, 1 is fully connected
	...
	int PoolWidH[MAX_LAYERS];  // Horizontal pooling width looking back to prior layer
	int PoolWidV[MAX_LAYERS];  // And vertical
    // N of inputs per neuron(including bias) from prior layer = prior depth * (2*HalfWidH+1) * (2*HalfWidV+1) + 1
    // A CONV layer has this many weights per layer(slice); a LOCAL layer has this times its nhid
	int n_prior_weights[MAX_LAYERS + 1];
	int n_hid_weights;  // Total number of all hidden weights; includes bias; does not include final layer
	int n_all_weights;  // As above, but also includes final weights
	int max_any_layer;  // Max number of neurons in any layer, including input and output
	double *weights;  // All 'n_all_weights' weights, including final weights, are here, inorder by layer
	...
	double *thr_poolmax_id[MAX_THREADS][MAX_LAYERS];
	double *thr_gradient[MAX_THREADS];
	double *thr_layer_gradient[MAX_THREADS][MAX_LAYERS + 1];
	// This preserve thresholds for testing after training
	int class_type;  // 1=split at zero; 2=split at median; 3=split at .33 and .67 quantiles; READ_SERIES.cpp sets, MODEL.cpp uses
	double median;
	double quantile_33;
	double quantile_67;
